---
name: cl-virality-scoring
description: The shared virality brain — score any content (video clip, written script, hook line, post idea, thumbnail concept) 0-100 on 8 weighted signals, adapted per content type. Medium-agnostic single source of truth used by cl-ai-clipping (score clip candidates), cl-script-evaluator (score written scripts), and cl-dialogue-story. Load this whenever you need to judge "is this viral?" or rank content by viral potential.
...
|---|---|
| **`cl-ai-clipping`** | Candidate clips inside a long video (which 45-90s window to cut) |
| **`cl-script-evaluator`** | A written script BEFORE it's filmed |
| **`cl-dialogue-story`** | Story-driven dialogue clips (same brain, richer output) |
...
`speaker_ratio`) — see the `cl-ai-clipping` skill for the clip output schema.
...
| Concern | Lives in |
|---|---|
| Clip duration sweet spots (45-90s), boundary trimming, overlap dedup, chunking long video | `cl-ai-clipping` |
| Audio-energy score boosts (peaks, silence-then-peak) | `cl-ai-clipping` (Phase 4.5) |
| Script templates, rewrite modes, hook rewriting, platform pacing | `cl-script-evaluator` |
| Turning a scored clip into an edited, captioned, reframed video | `cl-ai-clipping`, `cl-dialogue-story` |
