...
| Transcribe speech to text | `cl-ai-media` skill → `/api/bridge/ai/transcribe` (Whisper) — no separation needed |
...
- `cl-content-inspiration/social-scraping.md` — the front half of Recipe 2 (downloading reels via the SkillTown Desktop bridge). Prefer the bridge when the user is already signed in via the desktop app; use `yt-dlp --cookies-from-browser` as a fallback.
- `cl-ai-media/SKILL.md` — `/api/bridge/ai/transcribe` if you want text instead of audio.
- `cl-voice/SKILL.md` — voice cloning + TTS. Complementary — you can clone a voice from a `vocals_only.wav` produced here.
