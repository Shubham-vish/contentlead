...
> **⚠️ No bridge create route.** There is **no** `POST /api/bridge/content` to create a Content document. Create content with `POST /api/content/create` (documented in the `cl-editor` skill → `infrastructure.md`), then update it with `PUT /api/bridge/content/:id`.
