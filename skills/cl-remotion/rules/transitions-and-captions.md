...
> **Reference**: The editor supports 9 transition types (crossfade, slide-left/right/up/down, zoom-in/out, wipe, glitch) with automatic content-type→transition mapping. See `_Pipelines/pipeline/transition_planner.py` for the decision logic and `_Agent/skills/cl-remotion/rules/transitions-and-captions.md` for usage in SkillTown.
...
1. Generate voiceover: `POST /api/bridge/voice/generate` → audio URL (see `cl-voice` skill)
