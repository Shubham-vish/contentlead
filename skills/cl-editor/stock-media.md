...
| AI-generated novel images (no source available) | `cl-ai-media` → `/api/bridge/ai/image/generate` (Gemini) |
| Broad web image search (any site) | `cl-ai-media` → `/api/bridge/ai/image/search` (Tavily) |
...
- `cl-ai-media/SKILL` — Gemini image generation, GPT-4o vision analysis, Tavily search
