...
interface SampleScript {
  ...
  source: "manual" | "content-inspiration" | "content-editor";
...
### Base: `/api/content-style`

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/content-style/profiles` | List all style profiles |
| POST | `/api/content-style/profiles` | Create a new style profile |
| GET | `/api/content-style/profiles/[id]` | Get a specific profile |
| PUT | `/api/content-style/profiles/[id]` | Update a profile |
| DELETE | `/api/content-style/profiles/[id]` | Delete a profile |
| POST | `/api/content-style/profiles/[id]/analyze` | Re-analyze samples → update style |
| GET | `/api/content-style/profiles/[id]/samples` | List samples for a style |
| POST | `/api/content-style/profiles/[id]/samples` | Add a sample script |
| DELETE | `/api/content-style/profiles/[id]/samples/[sampleId]` | Remove a sample |
| POST | `/api/content-style/remix` | Remix inspiration → user's style |
| GET | `/api/content-style/remixes` | List past remixes |
| GET | `/api/content-style/remixes/[id]` | Get a specific remix |
...
```bash
curl -s -X POST "http://127.0.0.1:$PORT/api/bridge/content-style/profiles" \
...
```bash
curl -s -X POST "http://127.0.0.1:$PORT/api/bridge/content-style/profiles/abc-123/samples" \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s -X POST "http://127.0.0.1:$PORT/api/bridge/content-style/profiles/abc-123/analyze" \
...
```bash
curl -s -X POST "http://127.0.0.1:$PORT/api/bridge/content-style/remix" \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
  -d '{
...
load the **`virality-scoring`** skill (the single source of truth for the 8-signal 0–100 rubric)
and apply it to the remixed script. This keeps content-style, `ai-clipping`, and
`script-evaluator` all scoring against the same framework.

- If the remix scores **< 70**, iterate the hook/structure once and re-score before returning.
- For a full line-by-line rewrite pass (not just a score), hand the draft to **`script-evaluator`**
  Mode 2 — content-style owns *voice fidelity*, script-evaluator owns *craft polish*.
...
| Skill | Integration |
|-------|-------------|
| `virality-scoring` | **The scoring authority.** content-style does NOT define its own rubric — it calls this skill to score every remix (Step 4). Single source of truth. |
| `script-evaluator` | Craft polish + line-by-line rewrite of a remix draft (Mode 2). content-style owns voice fidelity; script-evaluator owns craft. |
| `content-inspiration` | Source of inspiration content to remix |
| `ai-clipping` | After filming the remixed script, clip it (shares the same `virality-scoring` brain) |
| `contentlead` | Create a video project from the remixed script |
| `content-direction` | Plan the visual treatment for the remix |
...
7. Scores it with the `virality-scoring` rubric (e.g. 72/100); if < 70, iterates the hook once and re-scores
8. Optionally: hands the draft to script-evaluator for a craft-polish rewrite pass
...
- **Composable** — remix output feeds into script-evaluator, contentlead, etc.
