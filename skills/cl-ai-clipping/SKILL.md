---
name: cl-ai-clipping
description: Extract viral short-form clips from long-form video. AI-powered transcript analysis, virality scoring (via the shared `cl-virality-scoring` skill), vertical reframing, and batch clip creation — all orchestrated through existing editor APIs.
...
- ContentLead Desktop running (follow `cl-editor` startup protocol)
...
> **`cl-dialogue-broll`** skill. It reuses this clip's word-level transcript and the
> `cl-ai-media` image tools.
...
**The scoring rubric lives in the standalone `cl-virality-scoring` skill — load it and apply it here.**
...
```
Load: skills/cl-virality-scoring/SKILL.md
Apply: Steps 1-5 (detect type → score 8 signals → composite 0-100 → adapt per type → checklist)
```

For clips specifically:
- **Content type** is usually `podcast` / `interview` / `commentary` / `debate` (see the profiles in `cl-virality-scoring`).
...
> cut. The *scoring itself* comes from the `cl-virality-scoring` skill.
...
defined in the **`cl-virality-scoring` skill, Step 4 — Content-Type Scoring Profiles**. Apply
