...
> Sourcing/analysis = the **`cl-ai-media`** skill (via `POST /api/bridge/ai/*`). Placement =
> the **`images`** / **`item-editing`** editor commands. Word timing = the
> **`transcription-and-editing`** / **`cl-ai-clipping`** skills. Load those for full arg
...
- You're building a **cl-dialogue-story** reel (that skill calls this same logic per line).
- You're finishing a **viral clip** (`cl-ai-clipping`) and want B-roll over the talking head.
...
- Whole clip already transcribed → reuse it (`cl-ai-clipping` Phase 1 /
...
See `cl-ai-media` §5 Transcription and `cl-ai-clipping` "Caption timing" for exact calls.
...
All LLM steps use **`POST /api/bridge/ai/text/generate`** (from `cl-ai-media`) with the prompts in the
...
> Image Search / Image Generation (see `cl-ai-media` §8).
...
- **Re-host first** if the URL is a temporary SAS/stock link:
  `POST /api/bridge/ai/asset/rehost` → permanent Azure URL (see `cl-ai-media` §6).
- **Z-ORDER:** B-roll must sit **in front of** the base video but **behind** captions/titles.
  ...
  > images > base video. See `cl-editor` "Track Z-Order" + `track-management`.
...
```
1. Ensure word timing (transcribe_with_speakers or reuse cl-ai-clipping transcript).
...
## Cross-references
- `cl-ai-media` — the actual `image/search` / `image/generate` / `image/analyze` /
  `text/generate` / `transcribe/*` / `asset/rehost` bridge routes (arg schemas + setup).
- `cl-dialogue-story` — full two-character reel pipeline; its Stage 1.5 IS this recipe wired
  into that format (ports the same TlEditingSolution engine).
- `cl-ai-clipping` — viral clip pipeline + caption presets; use this skill to add B-roll to a
  finished clip.
